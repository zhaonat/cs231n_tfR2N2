...
# data_dir = os.path.join(settings.ROOT_DIR, 'dataset');
...
class_name = 'planes'
filename = 'planes_cropped_grayscale/planes_02691156_4045_batch_cropped_grayscale'
X,y  = load_pickle.load(filename)
...
encoded_sequence = encoder.encoder_sequence(sequence)
...
for epoch in range(epochs):
    ...
    print('mean training_iou:', np.mean(training_iou))
    # print(np.mean(training_iou))

    # print('epoch: '+str(epoch)+' loss: '+str(loss_epoch))
    prediction = np.squeeze(prediction)
    print('mean training_acc:', np.mean(prediction == y_batch))
    loss_history.append(loss_epoch);
    toc = time.time()
    print('training time:', toc - tic)

    if(epoch%20 == save_every and epoch > 1): #save it occassionally in case we stop the run
    ...
    if(epoch % draw_every == 0):
        ...
        for j in range(mini_batch_size):
            ...
            plt.savefig('./results/' + 'sample_' + str(j) + '_epoch_' + str(epoch) + '.jpg')
            plt.close()
        print('figure saved!')
